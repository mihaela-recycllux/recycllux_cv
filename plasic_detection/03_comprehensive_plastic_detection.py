...
def detect_plastic_ml_clustering(indices, data_mask, water_mask, n_clusters=3):
    ...
    valid_water_mask = data_mask & water_mask
    ...
    pixel_mask = ~np.any(np.isnan(features_reshaped), axis=1) & valid_water_mask_flat
    ...
    cluster_labels_spatial[~water_mask] = -1
    ...
    confidence_scores_full[valid_mask] = confidence_scores_valid
...
def detect_plastic_anomaly_detection(indices, data_mask, water_mask):
    ...
    anomaly_scores_spatial[~water_mask] = 0
    anomaly_mask_spatial[~water_mask] = 0
    
    anomaly_pixels = np.sum(anomaly_mask_spatial == 1)
    total_water_pixels = np.sum(water_mask & data_mask)
...
def create_ensemble_detection(fdi_mask, ml_clusters, plastic_cluster_id, anomaly_mask, confidence_scores, water_mask):
    ...
    fdi_mask_water[~water_mask] = 0
    
    ml_mask_water = ml_mask.copy()
    ml_mask_water[~water_mask] = 0
    
    anomaly_mask_water = anomaly_mask.copy()
    anomaly_mask_water[~water_mask] = 0
    ...
    ensemble_confidence[water_mask] = confidence_scores[water_mask] * ensemble_score[water_mask]
    ensemble_confidence[~water_mask] = 0
    
    # Create final binary mask (ensemble detection > 0.5, water areas only)
    ensemble_mask = (ensemble_score > 0.5).astype(float)
    ensemble_mask[~water_mask] = 0
...
def calculate_plastic_area_statistics(detection_mask, bbox, water_mask=None):
    ...
    if water_mask is not None:
        ...
        # Count detections in water areas only
        detection_mask_water = detection_mask & water_mask
        ...
        # Original calculation for total area
    ...
    valid_pixels = ~np.isnan(detection_mask)
    detected_pixels = np.sum(detection_mask == 1)
    ...
    valid_area_m2 = total_valid_pixels * pixel_area_m2
    valid_area_km2 = valid_area_m2 / 1e6
    
    # Calculate percentages
    plastic_percentage = (detected_pixels / total_valid_pixels) * 100 if total_valid_pixels > 0 else 0
...
def create_multi_resolution_analysis(config, bbox, time_interval):
    ...
    for res_name, size in resolutions.items():
        ...
        try:
            ...
            fdi_mask = detect_plastic_fdi_method(indices, threshold=0.01)
            ml_clusters, plastic_cluster_id, confidence_scores = detect_plastic_ml_clustering(indices)
            anomaly_scores, anomaly_mask = detect_plastic_anomaly_detection(indices)
            ensemble_mask, ensemble_confidence = create_ensemble_detection(
                fdi_mask, ml_clusters, plastic_cluster_id, anomaly_mask, confidence_scores
            )
            
            # Calculate area statistics for each method
            area_stats = {
                'fdi': calculate_plastic_area_statistics(fdi_mask, bbox),
                'ml': calculate_plastic_area_statistics((ml_clusters == plastic_cluster_id).astype(float), bbox),
                'anomaly': calculate_plastic_area_statistics(anomaly_mask, bbox),
                'ensemble': calculate_plastic_area_statistics(ensemble_mask, bbox)
            }
...
• Bbox: {bbox.min_x:.1f}°E - {bbox.max_x:.1f}°E
    ...
    fdi_plot = ax1.imshow(detections['fdi_mask'], cmap=cmap_binary, vmin=0, vmax=1,
                         extent=[bbox.min_x, bbox.max_x, bbox.min_y, bbox.max_y])
    ax1.set_title(f'FDI Detection\n{fdi_area_stats["detected_area_m2"]:.0f} m² ({fdi_area_stats["plastic_percentage"]:.3f}%)', fontsize=12)
    ax1.set_xlabel('Longitude')
    ax1.set_ylabel('Latitude')
    ax1.grid(True, alpha=0.3)
    ...
    ml_plot = ax2.imshow(ml_mask, cmap=cmap_binary, vmin=0, vmax=1,
                        extent=[bbox.min_x, bbox.max_x, bbox.min_y, bbox.max_y])
    ax2.set_title(f'ML Clustering\n{ml_area_stats["detected_area_m2"]:.0f} m² ({ml_area_stats["plastic_percentage"]:.3f}%)', fontsize=12)
    ax2.set_xlabel('Longitude')
    ax2.set_ylabel('Latitude')
    ax2.grid(True, alpha=0.3)
    ...
    anomaly_plot = ax3.imshow(detections['anomaly_mask'], cmap=cmap_binary, vmin=0, vmax=1,
                             extent=[bbox.min_x, bbox.max_x, bbox.min_y, bbox.max_y])
    ax3.set_title(f'Anomaly Detection\n{anomaly_area_stats["detected_area_m2"]:.0f} m² ({anomaly_area_stats["plastic_percentage"]:.3f}%)', fontsize=12)
    ax3.set_xlabel('Longitude')
    ax3.set_ylabel('Latitude')
    ax3.grid(True, alpha=0.3)
    ...
    ensemble_plot = ax4.imshow(detections['ensemble_mask'], cmap=cmap_binary, vmin=0, vmax=1,
                              extent=[bbox.min_x, bbox.max_x, bbox.min_y, bbox.max_y])
    ax4.set_title(f'Ensemble Detection\n{ensemble_area_stats["detected_area_m2"]:.0f} m² ({ensemble_area_stats["plastic_percentage"]:.3f}%)', fontsize=12)
    ax4.set_xlabel('Longitude')
    ax4.set_ylabel('Latitude')
    ax4.grid(True, alpha=0.3)
...
• Total valid area: {ensemble_area_stats['valid_area_km2']:.2f} km²
• Plastic coverage: {ensemble_area_stats['plastic_percentage']:.3f}%
...
• Data coverage: {100 * ensemble_area_stats['total_valid_pixels'] / detections['ensemble_mask'].size:.1f}%
